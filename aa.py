...
TARGET_CHANNEL_ID = 1003063113224048741
...
async def on_message(message):
    ...
    # Parse the data from the embed message
    for embed in message.embeds:
        if embed.title == 'Police Shift':
            fields = embed.fields
            steam_name = fields[0].value.split(': ')[1]
            identifier = fields[1].value.split(': ')[1]
            shift_duration = int(fields[2].value.split(': ')[1].split()[0])
            start_date = datetime.strptime(fields[3].value.split(': ')[1], '%d/%m/%Y %H:%M:%S')
            end_date = datetime.strptime(fields[4].value.split(': ')[1], '%d/%m/%Y %H:%M:%S')
            username = message.author.name

            # Insert the data into the database
            conn = mysql.connector.connect(**config)
            cursor = conn.cursor()
            cursor.execute('INSERT INTO shift_logs (steam_name, identifier, shift_duration, start_date, end_date, username) VALUES (%s, %s, %s, %s, %s, %s)', (steam_name, identifier, shift_duration, start_date, end_date, username))
            conn.commit()
            cursor.close()
            conn.close()
@bot.event
